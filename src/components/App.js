...
class App extends Component {
 ...
 // Make Javascript version of smart contract
  async loadWeb3() {
    if(typeof window.ethereum!=='undefined' && !this.state.wrongNetwork){
      ...
      //when user changes account update account and balance
      ...
      //when user changes network update data
  ...
  async makeBet(bet, amount) {
    ...
    if(networkId!==4) {
      ...
      //send bet to the contract and wait for the result
      this.state.contract.methods.game(bet, randomSeed).send({from: this.state.account, value: amount}).on('transactionHash', (hash) => {
        ...
        this.state.contract.events.Result({}, async (error, event) => {
          ...
          //If user logouts while result is pending prevent error
